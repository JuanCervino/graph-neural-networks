            ...
            if trainStabilityEpsilon==0:
                ...
                for epoch in range(nEpochs):
                    ...
                    for batch in range(nBatches): # nBatches
                        ...
                        print(thisModel,batch,'/',nBatches,'in Epoch',epoch,'/',nEpochs,'of Fold',split,'/',nDataSplits,'Dual Variable',dualVariable)
                        ...
                        # Make sure that the correct matrix is in place
                        if '1Ly' in thisModel:
                            modelsGNN[thisModel].archit.changeGSO(S, [S.shape[0]])
                        elif '2Ly' in thisModel:
                            modelsGNN[thisModel].archit.changeGSO(S, [S.shape[0], S.shape[0]])
                        ...
                        # Compute loss
                        lossValueTrain = (1-dualVariable)*modelsGNN[thisModel].loss(yHatTrain, yTrain)
                        #
                        #Now implement the dual
                        for i in range(numberPerturbationsTrain):
                            # Create Perturbed Matrix
                            E = np.random.uniform(size=S.shape)
                            scale = trainStabilityEpsilon * np.random.uniform()
                            ...
                            # Compute loss
                            lossValueTrain += (dualVariable)*modelsGNN[thisModel].loss(yHatTrain, yTrain)
                    ...
                    yHatTrain = modelsGNN[thisModel].archit.singleNodeForward(xTrain, targetIDs)
                    # Compute loss
                    lossValueTrainNonPerturbed = modelsGNN[thisModel].loss(yHatTrain, yTrain).item()

                    lossValueTrainPerturbed=0
                    # Now implement the dual
                    for i in range(numberPerturbationsTrain):
                        # Create Perturbed Matrix
                        E = np.random.uniform(size=S.shape)
                        ...
                        # Compute loss
                        lossValueTrainPerturbed= lossValueTrainPerturbed + modelsGNN[thisModel].loss(yHatTrain, yTrain).item()
